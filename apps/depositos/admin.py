...
class DepositoAdmin(admin.ModelAdmin):
    list_display = ('codigo', 'nombre', 'tipo', 'tienda', 'ciudad', 'estado', 'fecha_creacion')
    list_filter = ('estado', 'tipo', 'tienda', 'ciudad', 'fecha_creacion')
    search_fields = ('nombre', 'codigo', 'ciudad', 'direccion', 'tienda__nombre')
    readonly_fields = ('creado_por', 'fecha_creacion', 'fecha_actualizacion')
    
    fieldsets = (
        ('Información Básica', {
            'fields': ('nombre', 'codigo', 'descripcion', 'tipo', 'tienda', 'estado')
        }),
        ('Ubicación', {
