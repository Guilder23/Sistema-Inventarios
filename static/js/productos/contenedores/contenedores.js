// Contenedores: búsqueda en tiempo real + paginación server-side
(function() {
    'use strict';
    ...
    function marcarAutofocusBuscador() {
        try {
            sessionStorage.setItem('contenedores_autofocus_buscar', '1');
        } catch (e) {
            // noop
        }
    }
    ...
    function restaurarFocoBuscadorSiCorresponde() {
        ...
        try {
            debeEnfocar = sessionStorage.getItem('contenedores_autofocus_buscar') === '1';
        } catch (e) {
            debeEnfocar = false;
        }
        ...
        try {
            sessionStorage.removeItem('contenedores_autofocus_buscar');
        } catch (e) {
            // noop
        }
    ...
    function inicializarBusquedaTiempoReal() {
        const inputBuscar = document.getElementById('buscar');
        const form = document.getElementById('formFiltrosContenedores');
